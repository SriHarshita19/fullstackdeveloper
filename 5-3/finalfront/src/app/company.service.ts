...
export class CompanyService {
    ...
    httpUrl = 'http://localhost:9000/companyall/';
    //company = Company;
   ...
   deleteCompanies(id:number):Observable<Company>{
     return this.ht.delete(`http://localhost:9000/deletecompany/${id}`);
   }
   updateCompaniesInfo(com :Company):Observable<Company>{
     return this.ht.put(`http://localhost:9000/updatecompany`,com);
   }
   getCompaniesById(id :number):Observable<Company>{
     return this.ht.get(`http://localhost:9000/companyid/${id}`);
   }
 }